...
function initSharedCursors(textElement) {
  ...
  textElement.on("reference", event => {
    if (event.reference.key() === cursorKey) {
...
function setLocalSelection() {
  if (!editor.selection.isEmpty()) {
    ...
    const indexRanges = aceRanges.map((aceRange) => {
      const start = doc.positionToIndex(aceRange.start);
      const end = doc.positionToIndex(aceRange.end);
