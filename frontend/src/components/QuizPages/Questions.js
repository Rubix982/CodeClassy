...
const Questions = (props) => {
  ...
  const updateSelectedQuestions = (e, id) => {
    ...
    if(e.target.checked)
    {
        selectedQuestionsId.push(id);
        console.log(selectedQuestionsId);
        setSelectedQuestionsId([...selectedQuestionsId])
    }
    else
    {
        const temp = selectedQuestionsId.filter(e => e != id);
        console.log(temp);
        setSelectedQuestionsId(temp);
    }
  }
          ...
          if (item.categoryID == category.ID) {
            return (
              <div className={QuestionsStyling.question} key={index}>
                  <Checkbox {...label} size="small" key={index} onChange={e => updateSelectedQuestions(e, item.ID)}/>
                  <div style={{display: 'flex', width: '100%', marginBottom: '10px', justifyContent: 'flex-start', alignItems: 'flex-start'}}>
                    <h4> {item.title}</h4>
                  </div>
                  <div className={QuestionsStyling.body}>
                    <p dangerouslySetInnerHTML={{ __html: draftToHtml(item.body) }}/> 
                  </div>
              </div>
            );
          }
